...
const PledgeTable = () => {
  ...
  // Fixed fetchData function with proper API implementation
  const fetchData = useCallback(async () => {
    setLoading(true);
    setError(null);
    ...
    try {
      ...
      const temp = rawData.map((item) => {
        return {
          ...
          udiseCode: item.udiseCode || item.udseCode || "",
          address: item.address || "",
          schoolType: item.schoolType || "",
          state: item.state || "",
          district: item.district || "",
          ...
          rapidVisualSurvey: item.rapidSurvay || item.rapidVisualSurvey || "N/A",
          disasterPlan: item.hasDMPlan || item.disasterPlan || "N/A",
          ...
          programLike: item.programLike || "N/A",
        };
      });
  ...
  <html>
    ...
    <body>
      <h1>School Registration Report</h1>
      <p style="text-align: center; color: #666; margin-bottom: 10px;">
        <strong>UDISE Code:</strong> ${rowData.udiseCode || "N/A"} | 
        <strong>Generated on:</strong> ${new Date().toLocaleDateString()}
      </p>
  ...
  if (error) {
    return (
      <div style={{ padding: "20px", textAlign: "center" }}>
        ...
        <Button onClick={fetchData} variant="contained" style={{ marginTop: "10px" }}>
          Retry
        </Button>
      </div>
    );
    ...
    <>
      {showRegistrationForm ? (
        ...
        <>
          <div style={{ padding: "20px" }}>
            <MaterialTable 
              title={`School Registration Data (${data.length} records)`} 
              columns={columns} 
              data={data} 
              options={{ 
                pageSize: 10, 
                      ...
                      <Grid item xs={12} md={8}>
                        ...
                        <Table size="small" sx={{ mb: 3 }}>
                          <TableBody>
                            ...
                            <TableRow>
                              <TableCell sx={{ fontWeight: "bold" }}>Male Students</TableCell>
                              <TableCell>{selectedRow?.maleStudents || "N/A"}</TableCell>
                            </TableRow>
                            <TableRow>
                              <TableCell sx={{ fontWeight: "bold" }}>Female Students</TableCell>
                              <TableCell>{selectedRow?.femaleStudents || "N/A"}</TableCell>
                            </TableRow>
                          </TableBody>
                        </Table>
                        ...
                        <Table size="small" sx={{ mb: 3 }}>
                          <TableBody>
                            ...
                            <TableRow>
                              <TableCell sx={{ fontWeight: "bold" }}>Male Teachers</TableCell>
                              <TableCell>{selectedRow?.maleTeachers || "N/A"}</TableCell>
                            </TableRow>
                            <TableRow>
                              <TableCell sx={{ fontWeight: "bold" }}>Female Teachers</TableCell>
                              <TableCell>{selectedRow?.femaleTeachers || "N/A"}</TableCell>
                            </TableRow>
                            <TableRow>
                              <TableCell sx={{ fontWeight: "bold" }}>Program Like</TableCell>
                              <TableCell>{selectedRow?.programLike || "N/A"}</TableCell>
                        ...
                        <Table size="small">
                          <TableBody>
                            <TableRow>
                              <TableCell sx={{ fontWeight: "bold" }}>Rapid Visual Survey Done?</TableCell>
                              <TableCell>{selectedRow?.rapidVisualSurvey || "N/A"}</TableCell>
                            </TableRow>
                            <TableRow>
                              <TableCell sx={{ fontWeight: "bold" }}>Disaster Management Plan Created?</TableCell>
                              <TableCell>{selectedRow?.disasterPlan || "N/A"}</TableCell>
              ...
              <Button
                onClick={async () => {
                  try {
                    ...
                    setData(updated);
                    setOpenEditDialog(false);
                    alert("Contact and email updated successfully!");
                  } catch (err) {
